...
Esta pieza es la bisagra entre Fase 4 y Fase 6. Un bloque no se registra solo
con su funcion: se registra con la **descripcion de sus parametros**. Gracias a
eso discovery puede enumerar y mutar el espacio de busqueda sin conocer ningun
bloque concreto y sin que nadie mantenga a mano una lista paralela de
combinaciones.

Anadir un indicador nuevo consiste en escribir la funcion y decorarla. El
buscador lo incorpora automaticamente en la siguiente corrida.
"""
...
class ParamSpec:
    ...
    Attributes:
        ...
        choices: Conjunto discreto de valores explorables por discovery. Es
            ...
            25 ni 30, no es un resultado.
        low / high: Cotas duras de validacion, aplicables tambien a valores que
            un humano fije a mano fuera de `choices`.
        description: Texto que explica que controla el parametro. Obligatorio
            en la practica: alimenta la documentacion de configuracion.
    """
    ...
    def validate(self, value: Any) -> None:
        ...
        Raises:
            ValueError: si el valor esta fuera de las cotas declaradas.
        """
...
class ComponentEntry(Generic[T]):
    ...
    Attributes:
        warmup_fn: Dado el diccionario de parametros, devuelve cuantas barras
            ...
            debe descartar ese prefijo en cada fold, y sin declararlo por
            componente habria que estimarlo, con riesgo de contaminar el
            out-of-sample.
        tags: Etiquetas de familia ("trend", "momentum", "volatility"). Permiten
            ...
            aportan redundancia y no diversificacion.
    """
    ...
    def resolve(self, overrides: Mapping[str, Any] | None = None) -> dict[str, Any]:
        ...
        Raises:
            ValueError: ante un parametro desconocido o fuera de cotas. Un
                ...
                de configuracion, y aceptarlo en silencio hace que la
                estrategia ejecutada no sea la que el usuario cree.
        """
...
class Registry(Generic[T]):
    ...
    El registro falla ruidosamente ante nombres duplicados. Un nombre ambiguo
    invalida cualquier artefacto que lo referencie, porque deja de estar claro
    que codigo se ejecuto.
    """
        ...
        *,
        ...
        def decorator(fn: T) -> T:
            ...
            self._entries[name] = ComponentEntry(
                ...
                tags=frozenset(tags),
                description=description or (getattr(fn, "__doc__", "") or "").strip().split("\n")[0],
            )
            return fn
    ...
    def catalog(self) -> list[dict[str, Any]]:
        ...
        Se persiste junto a cada corrida de discovery: sin el, no se puede
        saber que espacio de busqueda existia el dia que se hizo la busqueda.
        """
